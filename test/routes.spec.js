...
describe('Api Route Tests', () => {
  ...
  beforeEach( async () => {
    model = createModel()
    server = await startEditor({ models: [model], port: 3002})
  })
  ...
  it('should serve a new Editor', async () => {
    let result = await connect().get('/api/')
  ...
  it(`should serve model data on GET /api/${schema.title}/`, async () => {
    let result = await connect().get(`/api/${schema.title}/` )
    expect(result).to.have.status(200)
  ...
  it(`should not serve model data on model that does not exist`, async () => {
    let result = await connect().get(`/api/foo/` )
    expect(result).to.have.status(400)
  })

  it(`should serve model data on GET /api/${schema.title}/0`, async () => {
    let result = await connect().get(`/api/${schema.title}/0` )
    expect(result).to.have.status(200)
  ...
  it(`should delete entry data on DELETE /api/${schema.title}/0`, async () => {
    let result = await connect().delete(`/api/${schema.title}/0` )
    expect(result).to.have.status(200)
    result = await connect().get(`/api/${schema.title}/` )
    expect(result.body.data).to.be.lengthOf(2)
  })

  it(`should not be able to delete non existing entry`, async () => {
    let result = await connect().delete(`/api/${schema.title}/5` )
    expect(result).to.have.status(400)
  })

  it(`should be able to create a new entry`, async () => {
    const data = { id: 4, data: 'bar'}
    let result = await connect().post(`/api/${schema.title}/` ).send(data)
    expect(result).to.have.status(200)
  ...
  it(`should be able to change an entry`, async () => {
    const data = { data: 'bar'}
    var result = await connect().put(`/api/${schema.title}/0` ).send(data)
    result = await connect().get(`/api/${schema.title}/0` )
    expect(result).to.have.status(200)
  ...
  it(`should receive error, when updated entry does not exist`, async () => {
    const data = { data: 'bar'}
    const result = await connect().put(`/api/${schema.title}/5` ).send(data)
    expect(result).to.have.status(400)
  })
